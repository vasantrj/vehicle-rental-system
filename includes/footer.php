<footer class="site-footer">
  <div class="footer-inner">
    <div class="footer-top">
      <div>
        <div class="footer-brand">
          <div class="footer-brand-icon">🚗</div>
          <span style="background:linear-gradient(135deg,#fff,var(--orange-pale));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">DriveEase</span>
        </div>
        <p class="footer-desc">Premium vehicle rental with instant booking, transparent pricing, and 24/7 support across India.</p>
      </div>
      <div>
        <div class="footer-col-title">Explore</div>
        <a href="<?= $root ?>index.php" class="footer-link">Home</a>
        <a href="<?= $root ?>user/vehicles.php" class="footer-link">Vehicles</a>
        <a href="<?= $root ?>pages/about.php" class="footer-link">About Us</a>
        <a href="<?= $root ?>pages/contact.php" class="footer-link">Contact</a>
      </div>
      <div>
        <div class="footer-col-title">Account</div>
        <a href="<?= $root ?>auth/login.php" class="footer-link">Sign In</a>
        <a href="<?= $root ?>auth/register.php" class="footer-link">Register</a>
        <a href="<?= $root ?>user/my-bookings.php" class="footer-link">My Bookings</a>
        <a href="<?= $root ?>user/profile.php" class="footer-link">Profile</a>
      </div>
      <div>
        <div class="footer-col-title">Contact</div>
        <span class="footer-link">📧 support@driveease.in</span>
        <span class="footer-link">📞 +91 98765 43210</span>
        <span class="footer-link">📍 Bengaluru, India</span>
      </div>
    </div>
    <div class="footer-bottom">
      <span class="footer-copy">© <?= date('Y') ?> DriveEase. All rights reserved.</span>
      <span class="footer-badge">⚡ Built with PHP + MySQL</span>
    </div>
  </div>
</footer>
